...
public class MSimSignalClusterView
   ...
   /* private int[] mNoSimSlotResourceId = {R.id.no_sim, R.id.no_sim_slot2, R.id.no_sim_slot3};*/
    ...
    private ImageView mMobileSlotIndicatorSub2;
    private boolean mMobileDataContected[];
    ...
    public MSimSignalClusterView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mMobileDataContected = new boolean[mNumPhones];
    ...
    protected void onAttachedToWindow() {
        ...
        mMobileSlotIndicatorSub2 = (ImageView) findViewById(R.id.mobile_slot_indicator_sub2);
        ...
        getContext().registerReceiver(mSimStateReceiver, mIntentFilter);
    }

    private void updateSimIndicator() {
        Log.d(TAG, "LewaSimInfo list: " + SubscriptionManager.getActiveSubInfoCount());
        int subNum = SubscriptionManager.getActiveSubInfoCount();
        boolean mAirplaneMode = (Settings.Global.getInt(mContext.getContentResolver(),
                Settings.Global.AIRPLANE_MODE_ON, 0) == 1);
        boolean gemini = subNum > 1;
        Log.d(TAG, "mAirplaneMode: " + mAirplaneMode);
        if (subNum > 1 && !mAirplaneMode) {
            mMobileSlotIndicator.setVisibility(View.VISIBLE);
        } else {
            mMobileSlotIndicator.setVisibility(View.GONE);
        }
        if (subNum > 1 && !mAirplaneMode) {
            mMobileSlotIndicatorSub2.setVisibility(View.VISIBLE);
        } else {
            mMobileSlotIndicatorSub2.setVisibility(View.GONE);
        }
    }
...
//        }
        ...
        Slog.d("type", "mMobileTypeId:2223333 " +  mMobileTypeId[phoneId]+"phoneId:"+phoneId);
        ...
        mMobileDataContected[phoneId] = mobileDataContected;
    ...
    private void apply(int phoneId) {
        ...
        Slog.d("type", "mWifiVisible :" + mWifiVisible
                + "mMobileDataContected[phoneId] :"
                + mMobileDataContected[phoneId] + "phoneId:" + phoneId);
        if (mStyle == STATUS_BAR_STYLE_ANDROID_DEFAULT) {
            mMobileType[phoneId]
                    .setVisibility((!mWifiVisible
                            && mMobileDataContected[phoneId]) ? View.VISIBLE
                            : View.GONE);
        } else {
    ...
    private BroadcastReceiver mSimStateReceiver = new  BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            Log.d(TAG, "onReceive android.intent.action.SIM_STATE_CHANGED");
            updateSimIndicator();
        }
    };

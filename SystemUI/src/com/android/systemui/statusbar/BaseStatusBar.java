...
import com.android.systemui.notifilter.PopuViewForNotifications;
...
public abstract class BaseStatusBar extends SystemUI implements
    ...
    private final NotificationListenerService mNotificationListener =
        ...
        public void onListenerConnected() {
            if (true) Log.d(TAG, "onListenerConnected");
        ...
        public void onNotificationPosted(final StatusBarNotification sbn,
            ...
            if (true) Log.d(TAG, "onNotificationPosted: " + sbn);
            mHandler.post(new Runnable() {
                @Override
                public void run() {
        ...
        public void onNotificationRemoved(final StatusBarNotification sbn,
                final RankingMap rankingMap) {
            if (true) Log.d(TAG, "onNotificationRemoved: " + sbn);
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    if (sbn != null) {
                        removeNotification(sbn.getKey(), rankingMap);
                        Log.d(TAG,"sbn.getKey():"+sbn.getKey());
                    }
                }
            });
        }
        ...
        public void onNotificationRankingUpdate(final RankingMap rankingMap) {
            if (true) Log.d(TAG, "onRankingUpdate");
            mHandler.post(new Runnable() {
                @Override
                public void run() {
    ...
    public void start() {
         // add by jjlin for PR974759 begin
        Intent intentforhotknotinit = new Intent("com.lewa.hotknot.initialize");
        mContext.sendBroadcast(intentforhotknotinit);
        // add by jjlin for PR974759 end
    ...
    // LEWA Begin, added by yljiang@lewatek.com 2013-03-27
    ...
    public void showBirdView () {
        if (intentBirdView == null) {
            intentBirdView = new Intent("com.lewa.birdview.action.TOGGLE_RECENTS");
    ...
    private boolean inflateViews(NotificationData.Entry entry, ViewGroup parent, boolean isHeadsUp) {
        ...
        if (isHeadsUp) {
            maxHeight =
                    mContext.getResources().getDimensionPixelSize(R.dimen.heads_up_height);
        ...
        //very import
        row.setTag(sbn.getPackageName());
        Log.i(TAG, "sbn.getPackageName() :" + row.getTag());
        ...
        if (contentIntent != null) {
            final View.OnClickListener listener = makeClicker(contentIntent,
                    sbn.getKey(), isHeadsUp,sbn);
            final View.OnLongClickListener longlistener = makeClicker(
                    contentIntent, sbn.getKey(), isHeadsUp,sbn);
        ...
        int headsUpNotifications = mContext.getResources()
                .getDimensionPixelSize(R.dimen.heads_up_height);
        entry.row = row;
        entry.row.setHeightRange(mRowMinHeight, maxHeight,headsUpNotifications);
...
///Lewa end
    ...
    public NotificationClicker makeClicker(PendingIntent intent, String notificationKey,
            boolean forHun,StatusBarNotification notification) {
        return new NotificationClicker(intent, notificationKey,forHun,notification);
    }

    protected class NotificationClicker implements View.OnClickListener,View.OnLongClickListener {
        private PendingIntent mIntent;
        private final String mNotificationKey;
        private boolean mIsHeadsUp;
        public StatusBarNotification mNotification;

        public NotificationClicker(PendingIntent intent,String notificationKey,boolean forHun,StatusBarNotification notification) {
            mIntent = intent;
        ...
        public boolean onLongClick(View v) {
            ...
            Log.i(TAG,"mNotification.getPackageName() :"+mNotification.getPackageName());
            if (!mNotification.getPackageName().equals("android")) {
                mContext.startActivity(new Intent(
                        ...
                        .putExtra(Settings.EXTRA_APP_PACKAGE,
                                mNotification.getPackageName())
                        .putExtra(Settings.EXTRA_APP_UID,
                                mNotification.getUid()));
                animateCollapsePanels(CommandQueue.FLAG_EXCLUDE_NONE, true /* force */);
                return true;
            }
            return false;
...
//        mStackScroller.changeViewPosition(mDismissView, mStackScroller.getChildCount() - 1);
    ...
    private void updateNotificationViews(NotificationData.Entry entry,
        ...
        if (contentIntent != null) {
            final View.OnClickListener listener = makeClicker(contentIntent,
                    notification.getKey(), isHeadsUp, notification);
            final View.OnLongClickListener longlistener = makeClicker(
                    contentIntent, notification.getKey(), isHeadsUp,
                    notification);
            entry.row.setOnClickListener(listener);
            entry.row.setOnLongClickListener(longlistener);
        } else {
            entry.row.setOnClickListener(null);
            entry.row.setOnLongClickListener(null);
        }

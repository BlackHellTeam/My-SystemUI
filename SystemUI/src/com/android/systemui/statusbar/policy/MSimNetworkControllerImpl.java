...
import com.lewa.systemuiext.LewaSignalStrength;
...
public class MSimNetworkControllerImpl extends NetworkControllerImpl {
    ...
    public interface MSimSignalCluster {
        ...
        void setMobileDataIndicators(boolean visible, int strengthIcon, int activityIcon,
                ...
                int phoneId, int noSimIcon,boolean mobileDataContected);
    ...
    public void refreshSignalCluster(MSimSignalCluster cluster, int phoneId) {
        ...
        if (true) {
            Slog.d("type", "refreshSignalCluster, mMSimPhoneSignalIconId[" + phoneId + "]="
        ...
        if (mAirplaneMode) {
            ...
            switch (mMSimState[sub]) {
                ...
                case UNKNOWN:
                    ...
                    break;
                case NOT_READY:
                    textResId = com.android.internal.R.string.lockscreen_carrier_default;
                    break;
                case PIN_REQUIRED:
    ...
    private void setCarrierText() {
        String carrier1Name = mCarrierTextSub[PHONE_ID1];
        String carrier2Name = mCarrierTextSub[PHONE_ID2];
       // for (int i = 1; i < mPhoneCount; i++) {
            Slog.i(TAG,"carrier1Name :"+carrier1Name+"carrier2Name :"+carrier2Name);
       // }
        ...
        if (mContext.getResources().getBoolean(R.bool.config_showDataConnectionView)) {
            ...
            for (int i = 0; i < mMobileLabel2Views.size(); i++) {
                TextView v = mMobileLabel2Views.get(i);
                v.setText(carrier2Name);
                v.setVisibility(View.VISIBLE);
            }
        }
    }
    ...
    private PhoneStateListener getPhoneStateListener(long subId, int slotId) {
        PhoneStateListener mMSimPhoneStateListener = new PhoneStateListener(subId) {
            ...
            public void onSignalStrengthsChanged(SignalStrength signalStrength) {
                ...
                //PR978565 -haijun.chen-001 modify begin
                if(getPhoneId(mSubId) == SubscriptionManager.INVALID_PHONE_ID){
                    return ;
                }
                //PR978565 -haijun.chen-001 modify end
                mMSimSignalStrength[getPhoneId(mSubId)] = new LewaSignalStrength(signalStrength);
    ...
    protected void updateSimState(Intent intent) {
        ...
        Slog.d("lbj", "updateSimState for phoneId :" + phoneId+"stateExtra :"+stateExtra);
        if (phoneId >= 0) {
            ...
            }else if (IccCardConstants.INTENT_VALUE_ICC_NOT_READY
                    .equals(stateExtra)) {
            ...
            if (simState != IccCardConstants.State.UNKNOWN && simState != mMSimState[phoneId]) {
                ...
                updateCarrierText(phoneId);
                Slog.d("lbj", "updateSimState simState =" + mMSimState[phoneId]+"phoneId :"+phoneId);
            }
            updateIconSet(phoneId);
            updateDataIcon(phoneId);
            updateTelephonySignalStrength(phoneId);
    ...
    private final void updateTelephonySignalStrength(int phoneId) {
        Slog.d("lbj", "updateTelephonySignalStrength: phoneId =" + phoneId+"mMSimState[phoneId] :"+mMSimState[phoneId]);
        int dataSub = SubscriptionManager.getPhoneId(
                SubscriptionManager.getDefaultDataSubId());
        if (mMSimState[phoneId] == IccCardConstants.State.ABSENT
                || mMSimState[phoneId] == IccCardConstants.State.UNKNOWN) {
            Slog.d("lbj", " absent simcard");
            mMSimPhoneSignalIconId[phoneId] = 0;
            mMSimDataSignalIconId[phoneId] = 0;
            mQSPhoneSignalIconId = 0;
        }else if ((!hasService(phoneId) &&
                (mMSimDataServiceState[phoneId] != ServiceState.STATE_IN_SERVICE))
                || mMSimState[phoneId] == IccCardConstants.State.NOT_READY) {
            /*if (DEBUG)*/ Slog.d("lbj", " No service");
            mMSimPhoneSignalIconId[phoneId] =
            ...
            if (mMSimSignalStrength[phoneId] == null || (mMSimServiceState == null)) {
               /* if (DEBUG) {*/
                    Slog.d("lbj", " Null object, mMSimSignalStrength= "
...
//                if (DEBUG) {
                    Slog.d("lbj", "updateTelephonySignalStrength, sub: " + phoneId
                        + " level=" + iconLevel
...
//                }
    ...
    private final void updateDataNetType(int phoneId) {
        ...
        if (isCdma(phoneId)) {
            if (isCdmaEri(phoneId)) {
                mMSimDataTypeIconId[phoneId] = R.drawable.lewa_stat_sys_data_fully_connected_r_small;
                setQSDataTypeIcon = true;
                if (phoneId == dataSub) {
                    mQSDataTypeIconId = R.drawable.lewa_stat_sys_data_fully_connected_r_small;
                }
            }
    ...
    private final void updateDataIcon(int phoneId) {
        ...
        if (phoneId != dataSub) {
            ...
            if (DEBUG)Slog.d(TAG, "mMsimDataState=" + mMsimDataState +"phoneId:"+phoneId);
            if (mMsimDataState == TelephonyManager.DATA_CONNECTED) {
        ...
        if (mDataNetType == TelephonyManager.NETWORK_TYPE_UNKNOWN) {
            ...
            if (mMSimState[phoneId] == IccCardConstants.State.READY ||
                mMSimState[phoneId] == IccCardConstants.State.UNKNOWN) {
                mNoSim = false;
                Slog.d(TAG,"updateDataIcon  @@@@@@ =" + (mDataState == TelephonyManager.DATA_CONNECTED));
    ...
    void updateNetworkName(boolean showSpn, String spn, boolean showPlmn, String plmn,
        ...
        if (mMSimNetworkName[phoneId].equals("CU")) {
            mMSimNetworkName[phoneId] = mContext.getResources().getString(
                    R.string.simcard_name_cu);
        }
    ...
    protected void refreshViews(int phoneId) {
        ...
        if (!mHasMobileDataFeature) {
            ...
            if (mMSimDataConnected[phoneId]) {
                ...
                mMSimContentDescriptionCombinedSignal[phoneId] =
                        mMSimContentDescriptionDataType[phoneId];
            } else if(mMsimDataState == TelephonyManager.DATA_CONNECTED) {
                if (phoneId != dataSub) {
                    mMSimcombinedActivityIconId[phoneId] = mMSimMobileActivityIconId[phoneId];
                }
            }else{
        ...
        if (mAirplaneMode &&
            ...
            mAirplaneIconId = R.drawable.lewa_stat_sys_signal_flightmode;
        ...
        Slog.d("type", " mMSimDataConnected[phoneId]=" +  mMSimDataConnected[phoneId] +"phoneId:"+phoneId);
        if (!mMSimDataConnected[phoneId]) {
            mMSimDataTypeIconId[phoneId] = 0;
            if (phoneId == dataSub) {
                mQSDataTypeIconId = 0;
        ...
        if (mMSimLastPhoneSignalIconId[phoneId] != mMSimPhoneSignalIconId[phoneId]
         ...
         || mMSimLastcombinedActivityIconId[phoneId]
                != mMSimcombinedActivityIconId[phoneId])
        {
            Slog.d("type", " mMSimDataTypeIconId[phoneId]1111=" +  mMSimDataTypeIconId[phoneId] +"phoneId:"+phoneId);

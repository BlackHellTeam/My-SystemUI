...
public class HeadsUpNotificationView extends FrameLayout implements SwipeHelper.Callback, ExpandHelper.Callback,
    ...
    public boolean showNotification(NotificationData.Entry headsUp) {
        ...
        if (mHeadsUp != null) {
            ...
            if (mSnapshotDrawable != null) {
                mSnapshotDrawable.setIntrinsicWidth(mHeadsUp.row.getWidth());
                mSnapshotDrawable.setIntrinsicHeight(mHeadsUp.row.getNotificationHeight());
                mHeadsUp.row.setLayerType(View.LAYER_TYPE_HARDWARE, null);
                mHeadsUp.row.setBackground(mSnapshotDrawable);
                mSnapshotDrawable.start();
            }
            updateHeight(mHeadsUp.row.getNotificationHeight());
    ...
    private final ViewOutlineProvider CONTENT_HOLDER_OUTLINE_PROVIDER =
            new ViewOutlineProvider() {
        @Override
        public void getOutline(View view, Outline outline) {
            /*if (mHeadsUp == null || mHeadsUp.row == null) {
                return;
            }
            int outlineLeft = view.getPaddingLeft();
            int outlineTop = view.getPaddingTop();
            int width = view.getWidth();
            int height = mHeadsUp.row.getNotificationHeight();
            if (width < 2 || height < 2) {
                return;
            }
            updateHeight(height);
            if (mSnapshotDrawable != null) {
                mSnapshotDrawable.setIntrinsicWidth(width);
                mSnapshotDrawable.setIntrinsicHeight(height);
                mHeadsUp.row.setLayerType(View.LAYER_TYPE_HARDWARE, null);
                mHeadsUp.row.setBackground(mSnapshotDrawable);
                mSnapshotDrawable.start();
            }

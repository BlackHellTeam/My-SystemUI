...
import com.lewa.systemuiext.blur.ScreenBlurDrawable;
...
public class NotificationPanelView extends PanelView implements
    ...
    private ScreenBlurDrawable mSnapshotDrawable;
    ...
    private void setBlurView() {
        mSnapshotDrawable = new ScreenBlurDrawable(getContext(), WindowManager.LayoutParams.TYPE_WALLPAPER,
                 WindowManager.LayoutParams.TYPE_KEYGUARD_DIALOG);
        mSnapshotDrawable.setColorFilter(0xAA000000, PorterDuff.Mode.SRC_ATOP);
        mBlurBackground = (ImageView) findViewById(R.id.blur_background);
        if (mBlurBackground != null) {
            mBlurBackground.setLayerType(View.LAYER_TYPE_HARDWARE, null);
            mBlurBackground.setScaleType(ScaleType.MATRIX);
            mBlurBackground.setImageMatrix(getRotationMatrix());
            mBlurBackground.setImageDrawable(mSnapshotDrawable);
        }
    }
    ...
    public void setBrightnessMirror(BrightnessMirrorController c) {
        ...
        ToggleSlider brightnessSliderBottom = (ToggleSlider) findViewById(R.id.brightness_slider_bottom);
        brightnessSliderBottom.setMirror(mirror);
        brightnessSliderBottom.setMirrorController(c);
    }
    ...
    private Matrix getRotationMatrix() {
      ...
      Log.d(TAG, "rotationDegree : " + rotationDegree);
      if(rotationDegree == 0) {
          return m;
      }
      else{
          final int centerX = mSnapshotDrawable.getIntrinsicWidth()/2;
          final int centerY = mSnapshotDrawable.getIntrinsicHeight()/2;
          m.postTranslate(-centerX, -centerY);
          m.postRotate(rotationDegree);
          m.postTranslate(centerY, centerX);
          return m;
      }
    }
    ...
    protected void onFinishInflate() {
        ...
        //setBlurView();
        ///lewa end
        mHeader = (StatusBarHeaderView) findViewById(R.id.header_old);
        ...
        mBrightnessControllerBottom = new BrightnessController(getContext(),
                (ImageView) findViewById(R.id.brightness_icon_bottom),
                (ToggleSlider) findViewById(R.id.brightness_slider_bottom));
        ///wsliu end
...
/*        mKeyguardStatusView.setPivotX(getWidth() / 2);
    ...
    public void setVisibility(int visibility) {
        ...
        //     if (mSnapshotDrawable != null) {
        //          mSnapshotDrawable.start();
        //     }
        // } else {
        //     if (mSnapshotDrawable != null) {
        //         mSnapshotDrawable.stop();
        //     }
        // }
    ...
    private void positionClockAndNotifications() {
        ...
        //lewa begin
        mNotificationStackScroller.setIntrinsicPadding(0);
        //lewa end
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                ...
                initVelocityTracker();
                trackMovement(event);
                if (shouldQuickSwitchIntercept(mInitialTouchX, mInitialTouchY, 0)) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(h) > mTouchSlop && Math.abs(h) > Math.abs(x - mInitialTouchX)
                        && shouldQuickSwitchIntercept(mInitialTouchX, mInitialTouchY, h)) {
                    onQsExpansionStarted();
                    mInitialHeightOnTouch = mQsExpansionHeight;
                    mInitialTouchY = y;
                    mInitialTouchX = x;
    ...
    private boolean shouldQuickSwitchIntercept(float x, float y, float yDiff) {
        boolean onHeader = x >= mBottomPanel.getLeft() && x <= mBottomPanel.getRight()
                && y >= mBottomPanel.getTop() && y <= mBottomPanel.getBottom();
       return onHeader;
    }
    ...
    private void updateQsState() {
        ...
        mHeader.setVisibility((mQsExpanded || !mKeyguardShowing) ? View.GONE : View.GONE);
        ...
        if (!getResources().getBoolean(R.bool.config_showTaskManagerSwitcher)) {
            mQsPanel.setVisibility(expandVisually ? View.GONE : View.GONE);
        }
        mQsContainer.setVisibility(
                mKeyguardShowing && !expandVisually ? View.GONE : View.GONE);
...
///wsliu end
    ...
    private void updateKeyguardStatusBarVisibility() {
        mKeyguardStatusBar.setVisibility(mKeyguardShowing? VISIBLE : GONE);
    }

    public void setDozing(boolean dozing) {
        if (dozing == mDozing) return;
        mDozing = dozing;
       android.util.Log.i("luo","mDozing:"+mDozing);
        if (mDozing) {

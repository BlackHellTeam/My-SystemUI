...
///wsliu add
    public static final int MODE_INCALL_UI = 6;
    public static final int MODE_SOUND_REC = 7;
    public static final int MODE_WIFI_AP = 8;
    public static final int MODE_TORCH_UP = 9;
///wsliu end
...
///wsliu add
        if (mode == MODE_INCALL_UI) return "MODE_INCALL_UI";
        if (mode == MODE_TORCH_UP) return "MODE_TORCH_UP";
        if (mode == MODE_SOUND_REC) return "MODE_SOUND_REC";
        if (mode == MODE_WIFI_AP) return "MODE_WIFI_AP";
///wsliu end
...
///wsliu add
        private final int mIncallUi;
        private final int mWifiAp;
        private final int mTorchUp;
        private final int mSoundRec;
///wsliu end
        ...
        public BarBackgroundDrawable(Context context, int gradientResourceId) {
            ...
            if (DEBUG_COLORS) {
                ...
                mWarning = 0xffff0000;
///wsliu add
                mIncallUi = 0xffff0000;
                mWifiAp = 0xff00ff00;
                mSoundRec = 0xffffd700;
                mTorchUp = 0xff0000ff;
///wsliu end
            } else {
                ...
                ///wsliu add
                mIncallUi = res.getColor(R.color.status_remind_incall);
                mWifiAp = res.getColor(R.color.status_remind_wifiap);
                mSoundRec = res.getColor(R.color.status_remind_rec);
                mTorchUp = res.getColor(R.color.status_remind_light);
                ///wsliu end
...
///wsliu add
            else if (mMode == MODE_WIFI_AP) {
                targetColor = mWifiAp;
            } else if (mMode == MODE_TORCH_UP) {
                targetColor = mTorchUp;
            } else if (mMode == MODE_INCALL_UI ) {
                targetColor = mIncallUi;
            } else if (mMode == MODE_SOUND_REC) {
                targetColor = mSoundRec;
            }
///wsliu end
            else {
                targetColor = mOpaque;

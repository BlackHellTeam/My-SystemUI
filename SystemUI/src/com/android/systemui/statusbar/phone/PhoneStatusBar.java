...
import com.android.systemui.statusbar.policy.StatusRemindController;
import com.android.systemui.statusbar.policy.StatusRemindController.StatusStateChangeCallback;
...
import com.android.systemui.statusbar.policy.BatteryControllerLewa;
...
import static com.android.systemui.statusbar.policy.StatusRemindController.INCALL_UI_NAME;
import static com.android.systemui.statusbar.policy.StatusRemindController.TORCH_UP_NAME;
import static com.android.systemui.statusbar.policy.StatusRemindController.WIFI_AP_NAME;
import static com.android.systemui.statusbar.policy.StatusRemindController.SOUND_REC_NAME;
...
///wsliu add
    ...
    StatusRemindController mStatusRemindController;
    private View mStatusRemind;
    private TextView mRemindText;
    private String mRemindPackage;
    private int mBarMode = -1;
    private int mTimeCount = 0;
    private Intent mRemindIntent;
    private int mTouchY = 0;
    private int mTouchX = 0;
    private static final float STATUS_BAR_RIGHT_ICON_SCALE = 0.85f;
///wsliu end
    ...
    private TextView mCarrier1 = null;
    private TextView mCarrier2 = null;
    private LinearLayout mCarrierLabelGemini = null;
    protected ImageView mSettingsButton;
    protected ImageView mClearButton;
    protected ImageView mNotifilterButton;
    protected CloseDragHandle mCloseDragHandle;
    private SharedPreferences mStatusbar_settings;
    BatteryControllerLewa mBatteryControllerLewa;
    private static HashSet mLewaAppPackageName;
...
///wsliu add
        ...
        mNotificationPanel.setBrightnessMirror(mBrightnessMirrorController);
///wsliu end
       ...
       // mStackScroller.addView(mKeyguardIconOverflowContainer);
        ...
        //lewa begin
        initAppPackageName();
        mSettingsButton =(ImageView) mNotificationPanel.findViewById(R.id.header).findViewById(R.id.settings_button);
        mSettingsButton.setOnClickListener(mButtonListener);
        mNotifilterButton =(ImageView) mNotificationPanel.findViewById(R.id.header).findViewById(R.id.notifilter_button);
        mNotifilterButton.setOnClickListener(mButtonListener);
        mClearButton=(ImageView) mNotificationPanel.findViewById(R.id.header).findViewById(R.id.clear_all_button);
        mClearButton.setOnClickListener(mButtonListener);
        mClearButton.setAlpha(0f);
        mClearButton.setVisibility(View.GONE);
        mClearButton.setEnabled(false);
        mCloseDragHandle = (CloseDragHandle) mNotificationPanel.findViewById(R.id.close);
        mCloseDragHandle.mService = this;
        mBatteryControllerLewa = new BatteryControllerLewa(mContext);
        mBatteryControllerLewa.addIconView((ImageView) mStatusBarWindow.findViewById(R.id.battery));
        mBatteryControllerLewa.addBatteryChargeView((ImageView)mStatusBarWindow.findViewById(R.id.battery_charge));
        mBatteryControllerLewa.addComboView((ViewGroup)mStatusBarWindow.findViewById(R.id.battery_combo));
        //lewa end
        mBackdrop = (BackDropView) mStatusBarWindow.findViewById(R.id.backdrop);
        mBackdropFront = (ImageView) mBackdrop.findViewById(R.id.backdrop_front);
        mBackdropBack = (ImageView) mBackdrop.findViewById(R.id.backdrop_back);
///wsliu add begin
        mStatusRemind = (View) mStatusBarWindow.findViewById(R.id.status_bar_remind);
        mRemindText = (TextView) mStatusRemind.findViewById(R.id.status_bar_remind_text);
///wsliu add end
        ...
        mHeader = (StatusBarHeaderView) mStatusBarWindow.findViewById(R.id.header_old);
        ...
        if (mTickerEnabled) {
            ...
            if (tickerStub != null) {
                ...
                mTicker = new MyTicker(context, mStatusBarView,mLewaAppPackageName);
...
///wsliu add remind controller
        mStatusRemindController = new StatusRemindController(mContext);
        mStatusRemindController.addStateChangedCallback(new StatusStateChangeCallback() {
            @Override
            public void onStatusRemindChanged(final Intent intent,final int mode) {
                Log.d(TAG,"onStatusRemindChanged......");
                //if (intent == null || paName == null) return;
                mBarMode = mode;
                mRemindIntent = intent;
                setStatusClicker();
                mHandler.post(mSetBarModes);
            }
            @Override
            public void onStatusRemindMode(final int mode) {
                mRemindIntent = null;
                mHandler.post(mBarModesCheck);
            }
            @Override
            public void onStatusTimeChanged(final int mode,int time){
                mTimeCount = time;
                if (mode == MODE_SOUND_REC) {
                    String recordString = DateUtils.formatElapsedTime(time);
                    String textString = mStatusRemindController.getStatusRemindText();
                    if (mRemindText != null) {
                        mRemindText.setText(textString+recordString);
                    }
                }
            }
            @Override
            public void onStatusCallTimeChanged(final int mode,int time){
                if (mode == MODE_INCALL_UI) {
                    String callString = DateUtils.formatElapsedTime(time);
                    String textString = mStatusRemindController.getStatusRemindText();
                    if (mRemindText != null) {
                        mRemindText.setText(textString+callString);
                    }
                }
            }
        });
///wsliu add end
        ...
        if (isMSim()) {
            ...
            mCarrier1 = (TextView) mStatusBarWindow.findViewById(R.id.carrier1);
            mCarrier2 = (TextView) mStatusBarWindow.findViewById(R.id.carrier2);
            ...
            mCarrierLabelGemini.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View arg0) {
                    // TODO Auto-generated method stub
                    List<LewaSimInfo> list = LewaSimInfo.getInsertedSimList(context);
                    if(list.size() > 1){
                        Intent intent = new Intent("com.android.settings.multisimsettings.MultiSimSettings");
            ...
            mMSimNetworkController.addMobileLabelView(mCarrier1);
            mMSimNetworkController.addMobileLabel2View(mCarrier2);
        ...
        /*    if (mShowCarrierInPanel) {
            ...
            mCarrier1 = (TextView) mStatusBarWindow.findViewById(R.id.carrier1);
            mCarrier1.setVisibility(mCarrierLabelVisible ? View.VISIBLE : View.INVISIBLE);
            mNetworkController.addMobileLabelView(mCarrier1);
           // mCarrierLabel = (TextView)mStatusBarWindow.findViewById(R.id.carrier_label);
           // mShowCarrierInPanel = (mCarrierLabel != null);
...
//                    updateCarrierLabelVisibility(false);
        ...
        ((BatteryMeterView) mStatusBarView.findViewById(R.id.battery_old)).setBatteryController(
                mBatteryController);
        ...
        filter.addAction("com.android.action.STATUS_SOUND_REC");
    ...
    private void addHeadsUpView() {
        int headsUpHeight = mContext.getResources()
                .getDimensionPixelSize(R.dimen.heads_up_height);
        WindowManager.LayoutParams lp = new WindowManager.LayoutParams(
    ...
    public void addIcon(String slot, int index, int viewIndex, StatusBarIcon icon) {
        ...
        if (slot.equals("ime")) {
            view.setScaleX(STATUS_BAR_RIGHT_ICON_SCALE);
            view.setScaleY(STATUS_BAR_RIGHT_ICON_SCALE);
        ...
        view.set(icon);
        mStatusIcons.addView(view, viewIndex, new LinearLayout.LayoutParams(mStatusIconSize, mStatusIconSize));
    ...
    private void updateNotificationIcons() {
        final LinearLayout.LayoutParams otherParams
            = new LinearLayout.LayoutParams(mStatusIconSize, mStatusIconSize);
        //otherParams.setMargins(0, 0, 5, 0);
        final LinearLayout.LayoutParams syetmParams
            = new LinearLayout.LayoutParams(mStatusIconSize,mStatusIconSize);
        ...
        for (int i=0; i<toShow.size(); i++) {
            ...
            if (v.getParent() == null) {
                /*String packageName = v.getStatusBarIcon().iconPackage;
                Log.i("TAG","packageName ---"+mLewaAppPackageName.contains(packageName));
                Log.i("TAG","packageName packageName---"+packageName);
                if (mLewaAppPackageName != null && mLewaAppPackageName.contains(packageName)) {
                    mNotificationIcons.addView(v, i, syetmParams);
                } else {
                    mNotificationIcons.addView(v, i, otherParams);
                }*/
                mNotificationIcons.addView(v, i, syetmParams);
                v.setTag(v.getStatusBarIcon().iconId);
            }
        }
     ...
     // remove same icons for we chat.
        final int childCountNew = mNotificationIcons.getChildCount();
        for (int i = 0; i < childCountNew - 1; i++) {
            for (int j = childCountNew - 1; j > i; j--) {

                if ((mNotificationIcons.getChildAt(j) != null)
                        && mNotificationIcons.getChildAt(i) != null) {
                    if (mNotificationIcons.getChildAt(j).getTag()
                            .equals(mNotificationIcons.getChildAt(i).getTag())) {
                        mNotificationIcons.removeView(mNotificationIcons
                                .getChildAt(j));
                    }
                }
            }
        ...
        public void onClick(View v) {
            if (v == mSettingsButton) {
                startActivity(new Intent(
                        android.provider.Settings.ACTION_SETTINGS), true /* dismissShade */);
            } else if (v == mClearButton) {
                clearAllNotifications();
            } else if (v == mNotifilterButton) {
                Intent i = new Intent(Constants.NOTIFILTER_ACTION);
    ...
    void makeExpandedVisible(boolean force) {
        ...
        if (mContext.getResources().getBoolean(
                com.android.internal.R.bool.config_statusBarTurnOffLeds)) {
        ...
        if(!mNotificationPanel.isFullyExpanded()){
            animateStatusBarShow(mSystemIconArea, false); 
        }
    }
...
//        mNotificationPanel.closeQs();
    ...
    public void setSystemUiVisibility(int vis, int mask) {
        ...
        if (diff != 0) {
            ...
            final boolean totalTrans = mStatusbar_settings.getInt(PhoneStatusbarSettings.KEY_TRANS_STATUSBAR_SETTING, 1) != 0;
            Log.i(TAG,"totalTrans :"+totalTrans+isHome());
            int sbMode = MODE_OPAQUE;
            if(isHome() && totalTrans){
              sbMode = MODE_TRANSPARENT;
            }else{
              sbMode = computeBarMode(oldVal, newVal, mStatusBarView.getBarTransitions(),
    ...
    private int computeBarMode(int oldVis, int newVis, BarTransitions transitions,
        ...
        Log.i(TAG,"oldMode :"+oldMode+"---"+newMode);
        if (oldMode == newMode) {
    ...
    private void checkBarModes() {
        if (mDemoMode) return;
        ///wsliu add
        if (mStatusRemindController.getStatusRemindMode()) return;
        ///wsliu end
...
///wsliu add begin
    private final Runnable mSetBarModes = new Runnable() {
        @Override
        public void run() {
            setBarMode();
        }
    ...
    private final Runnable mBarModesCheck = new Runnable() {
        @Override
        public void run() {
            removeBarMode();
        }
    ...
    private void setStatusClicker() {
        if (mStatusRemind != null) {
            mStatusRemind.setOnTouchListener(new View.OnTouchListener() {
                    @Override
                    public boolean onTouch(View v, MotionEvent event) {
                         if (mRemindIntent != null) {
                              postStartSettingsActivity(mRemindIntent,FADE_KEYGUARD_START_DELAY);
                              return true;
                        }
                        return false;
                    }
            ...
            mStatusBarView.setOnTouchListener(new View.OnTouchListener() {
                    ...
                    public boolean onTouch(View v, MotionEvent event) {
                        switch(event.getAction()) {
                            ...
                            break;
                            case MotionEvent.ACTION_UP:
                            if (Math.abs(mX - (int)event.getX()) < 2.0f && Math.abs(mY - (int)event.getY()) < 2.0f) {
                                if (mRemindIntent != null) {
                                    postStartSettingsActivity(mRemindIntent,FADE_KEYGUARD_START_DELAY);
                                    return true;
                                }
    ...
    public void updateBarHeight(int barHeight) {
        if (mStatusBarWindowManager != null) {
            mStatusBarWindowManager.setStatusBarHeight(barHeight);
        }
    }

    private void removeBarMode() {
        if (mStatusRemind != null && mRemindText != null) {
             mRemindText.setText("");
             mStatusRemind.setVisibility(View.GONE);
         }
        updateBarHeight(getStatusBarHeight());
        checkBarModes();
    }

    private int getStatusHeightSettings() {
        if (mContext == null) return getStatusBarHeight();
        ContentResolver cr = mContext.getContentResolver();
        int sh = Settings.System.getInt(cr,STATUS_REMIND_HEIGHT, getStatusBarHeight());
        return sh;
    }

    private FastColorDrawable getStatusRemindColor() {
        if (mStatusBarView != null && mContext != null){
            int mode = -1;
            final Resources res = mContext.getResources();
            switch (mStatusBarView.getBarTransitions().getMode()) {
                case MODE_SOUND_REC:
                mode = R.color.status_remind_rec;
                break;
                case MODE_INCALL_UI:
                mode = R.color.status_remind_incall;
                break;
                case MODE_TORCH_UP:
                mode = R.color.status_remind_light;
                break;
                case MODE_WIFI_AP:
                mode = R.color.status_remind_wifiap;
                break;
                default:
                mode = R.color.system_bar_background_transparent;
                break;
            }
            return new FastColorDrawable(res.getColor(mode));
        }
        return new FastColorDrawable(0xff000000);
    }

    private void setBarMode() {
        if (mBarMode < MODE_INCALL_UI) return;
        updateBarHeight(getStatusHeightSettings());
        if (mStatusBarView != null) {
            mStatusBarView.getBarTransitions().transitionTo(mBarMode, false);
        }
        if (mStatusRemind != null) {
            mStatusRemind.setVisibility(View.VISIBLE);
            mStatusRemind.setBackground(getStatusRemindColor());
            if (mRemindText != null ) {
                if (mBarMode == MODE_SOUND_REC) {
                    String callString = DateUtils.formatElapsedTime(mTimeCount);
                    String textString = mStatusRemindController.getStatusRemindText();
                    if (mRemindText != null) {
                        mRemindText.setText(textString+callString);
                    }
                } else {
                    if (!mStatusRemindController.getCallTimeVisible()) {
                        mRemindText.setText(mStatusRemindController.getStatusRemindText());
                    }
                }
            }
        }
...
///wsliu add end
    ...
    private class MyTicker extends Ticker {
        MyTicker(Context context, View sb,HashSet lewaAppPackageName) {
            super(context, sb,lewaAppPackageName);
            if (!mTickerEnabled) {
    ...
    public void updateExpandedViewPos(int thingy) {
        ...
        // int visiableHeight = getStatusBarHeight();
        // Region region = new Region(0, mDisplayMetrics.heightPixels, mDisplayMetrics.widthPixels, mDisplayMetrics.heightPixels);
        // mWindowManager.setViewTransparentRegion(mStatusBarWindow, region);
    ...
    protected void loadDimens() {
        final Resources res = mContext.getResources();
        mStatusIconSize = res.getDimensionPixelSize(R.dimen.status_bar_noti_icon_size);
...
//            Log.d(TAG, "size=" + newIconSize + " padding=" + newIconHPadding);
    ...
    public boolean isHome() {
        ...
        boolean isLauncherClass = rti.get(0).topActivity.getClassName().equals("com.lewa.launcher.Launcher");
    ...
    private void initAppPackageName(){
        mLewaAppPackageName = new HashSet();
        mLewaAppPackageName.add("android");
        mLewaAppPackageName.add("com.android.settings");
        mLewaAppPackageName.add("com.android.systemui");
    }
    public static HashSet getAppPackageName(){
        return mLewaAppPackageName;
    }
}

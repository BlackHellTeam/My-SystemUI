...
public class NetworkOverLimitActivity extends Activity {
    ...
    private void setMobileDataState(boolean enable, int stateValue) {
        if (TelephonyHelper.supportMultipleSims()) {
            int phoneId = SubscriptionManager.getDefaultDataPhoneId();
            Log.d("NetworkOverLimitActivity", "Set Data  for phoneId" + phoneId);
            Settings.Global.putInt(getContentResolver(),
                    Settings.Global.MOBILE_DATA + phoneId, stateValue);
            long[] subId = SubscriptionManager.getSubId(phoneId);
            mTelephonyManager.setDataEnabledUsingSubId(subId[0], enable);
        } else {
            Settings.Global.putInt(getContentResolver(),
                    Settings.Global.MOBILE_DATA, stateValue);
        }
    }

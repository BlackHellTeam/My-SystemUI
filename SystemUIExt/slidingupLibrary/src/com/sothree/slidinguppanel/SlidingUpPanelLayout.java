...
public class SlidingUpPanelLayout extends ViewGroup {
    ...
    private final static boolean DEBUG_GESTURE = SystemProperties.getBoolean("slidingpanellayout.debuggesture", false);
    protected final static boolean DEBUG_LAYOUT = SystemProperties.getBoolean("slidingpanellayout.debugsize", false);

    protected SlideState mSlideState = SlideState.COLLAPSED;
    ...
    public void setPanelHeight(int val) {
        setPanelHeightInner(val);
        requestLayout();
    }
    
    protected void setPanelHeightInner(int val) {
        mPanelHeight = val;
    }
    ...
    public int getCurrentParalaxOffset() {
        ...
        int offset = (int)(mSlideRange * Math.max(mSlideOffset, 0));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if(DEBUG_LAYOUT) {
            ...
            Log.d(TAG, "mSlideState : "  + mSlideState);
        }
        ...
        if(DEBUG_LAYOUT) {
        Log.d(TAG, "mMainView : " + mMainView + " : mSlideableView : " + mSlideableView);
        Log.d(TAG, "mSlideableView.getVisibility() : "  + mSlideableView.getVisibility());
        }
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child == mMainView && mSlideState != SlideState.HIDDEN) {
                height -= mPanelHeight;
            }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if(DEBUG_LAYOUT) {
                Log.d(TAG, "child : " + i + " rect: " + childLeft + ", " + childTop + ", " + childRight + ", " + childBottom);
            }
   ...
   //lewa add end
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (h != oldh) {
            if(DEBUG_GESTURE) {
                Log.d(TAG, "size changed from " + oldh + "  to  " + h);
            }
            mFirstLayout = true;
        }
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (!isEnabled() || !mIsSlidingEnabled || (mIsUnableToDrag && action != MotionEvent.ACTION_DOWN)) {
            mDragHelper.cancel();
            if(DEBUG_GESTURE) {
                Log.d(TAG, "isEnabled" + isEnabled() + "  mIsSlidingEnabled : " + mIsSlidingEnabled + " mIsUnableToDrag  " + mIsUnableToDrag );
            }
            return false;
        }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mIsUsingDragViewTouchEvents) {
                    if(DEBUG_LAYOUT)
                    Log.d(TAG, "mIsUsingDragViewTouchEvents");
...
///LEWA BEGIN
                if ((ady > dragSlop && adx > ady) || !isSlideViewUnder((int) x, (int) y)) {
                    mDragHelper.cancel();
...
///LEWA END
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        if(DEBUG_GESTURE) {
            Log.d(TAG, "onTouchEvent + " + MotionEvent.actionToString(ev.getAction()) + " isSlidingEnabled : " + isSlidingEnabled());
        }
        if(!isEnabled()) {
        ...
        if(DEBUG_GESTURE) {
            Log.d(TAG, "mDragHelper.processTouchEvent");
        }
        //pr59787 Lewa modify begin
        //fix SystemUI FC due Exception throw
        try {
        ...
        switch (action & MotionEventCompat.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (dx * dx + dy * dy < slop * slop &&
                    ...
                    if(isPanelSettling()) {
                        if(mSlideOffset+ 0.1 > mAnchorPoint) {
                            collapsePanel();
                        }
                        else {
    ...
    private boolean isSlideViewUnder(int x, int y) {
        View dragView = mSlideableView;
        if (dragView == null) return false;
        int[] viewLocation = new int[2];
        dragView.getLocationOnScreen(viewLocation);
        int[] parentLocation = new int[2];
    ...
    private boolean expandPanel(View pane, int initialVelocity, float mSlideOffset) {
        //modify for bug:#65144
        return smoothSlideTo(mSlideOffset, initialVelocity) || mFirstLayout;
    }
    ...
    private float computeSlideOffset(int topPosition) {
        ...
        // top position
        if(DEBUG_LAYOUT) {
        Log.d(TAG, "topBoundCollapsed: "  + topBoundCollapsed);
    ...
    public boolean isPanelSettling() {
        return mSlideState == SlideState.SETTLING;
    }
    ...
    public void hidePanel() {
        if (mSlideState == SlideState.DRAGGING || mSlideState == SlideState.SETTLING || mSlideState == SlideState.HIDDEN) return;
        int newTop = computePanelTopPosition(0.0f) + (mIsSlidingUp ? +mPanelHeight : -mPanelHeight);
 ...
 //lewa add begin
            mMainView.setAlpha((1 + mainViewOffset/(float)(mSlideRange * mAnchorPoint)));
 //lewa add end
            //lewa modified for bug:#65184
             mFirstLayout = false;
        }
    ...
    private ViewDragHelper.Callback mDragHelperCallback = new ViewDragHelper.Callback() {
        ...
        public void onViewDragStateChanged(int state) {
            ...
            else if (mDragHelper.getViewDragState() == ViewDragHelper.STATE_SETTLING) {
                mSlideState = SlideState.SETTLING;
            }
        }

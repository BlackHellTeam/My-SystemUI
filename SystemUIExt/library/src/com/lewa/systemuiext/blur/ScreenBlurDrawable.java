package com.lewa.systemuiext.blur;
...
public class ScreenBlurDrawable extends Drawable implements Runnable, Animatable {
    ...
    public void draw(Canvas canvas) {
        ...
        if (mBlurredBitmaps[preUpdatedIndex] == null) {
            // Nothing to draw
            // Bitmap defMap = Bitmap.createBitmap(10, 10, Bitmap.Config.ARGB_8888);
            // Bitmap blurred = Bitmap.createBitmap(defMap.getWidth(), defMap.getHeight(), Bitmap.Config.ARGB_8888);
            // defMap.eraseColor(Color.DKGRAY);
            // ImageUtils.fastBlur(defMap, blurred, 12);
            // mPaint.setAlpha(200);
            // canvas.drawBitmap(blurred, null, mDstRect, mPaint);
            return;
        }
    ...
    private class SnapshotTask extends AsyncTask<Void, Integer, Bitmap> {
        ...
        protected Bitmap doInBackground(Void... voids) {
            ...
            int height = (int)(mHeight/16) - 1;
            if (height > snapshot.getHeight()) 
                return snapshot;

            Bitmap snapshot2 = Bitmap.createBitmap(snapshot, 1, 1, snapshot.getWidth() - 1, height);
            Bitmap blurred = Bitmap.createBitmap(snapshot2.getWidth(), snapshot2.getHeight(), Bitmap.Config.ARGB_8888);

            ImageUtils.fastBlur(snapshot2, blurred, 12);
            //Bitmap outBlur = FastBlur.doBlurJniBitMap(snapshot2, 12,true);
            Log.d("liuwenshuai", "snapshot screen & blur image with size(" + mWidth + ", " + mHeight + ")" + " cost " +

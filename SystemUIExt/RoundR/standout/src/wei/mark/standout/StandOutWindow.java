 ...
 *         <github.com/jasonconnery> Edited by: Mohammad Adib
 */
public abstract class StandOutWindow extends Service {
    ...
    public static final String ACTION_REALLY_HIDE = "REALLY_HIDE";

    public static final String ACTION_REALLY_UNHIDE = "REALLY_UNHIDE";
    ...
    public static void reallyHide(Context context, Class<? extends StandOutWindow> cls, int id) {
        Log.d(TAG, "reallyHide startService");
        context.startService(getReallyHideIntent(context, cls, id));

    }

    public static void reallyUnhide(Context context, Class<? extends StandOutWindow> cls, int id) {
        context.startService(getReallyUnhideIntent(context, cls, id));
    }
     ...
     */
    public static void sendData(Context context, Class<? extends StandOutWindow> toCls, int toId,
            int requestCode, Bundle data, Class<? extends StandOutWindow> fromCls, int fromId) {
        context.startService(getSendDataIntent(context, toCls, toId, requestCode, data, fromCls,
                fromId));
    }
    ...
    public static Intent getReallyHideIntent(Context context, Class<? extends StandOutWindow> cls,
            int id) {
        return new Intent(context, cls).putExtra("id", id).setAction(ACTION_REALLY_HIDE);
    }

    public static Intent getReallyUnhideIntent(Context context,
            Class<? extends StandOutWindow> cls, int id) {
        return new Intent(context, cls).putExtra("id", id).setAction(ACTION_REALLY_UNHIDE);
    }
     ...
     *         {@link Context#startService(Intent)}.
     */
    public static Intent getSendDataIntent(Context context, Class<? extends StandOutWindow> toCls,
            int toId, int requestCode, Bundle data, Class<? extends StandOutWindow> fromCls,
            int fromId) {
        return new Intent(context, toCls).putExtra("id", toId).putExtra("requestCode", requestCode)
                .putExtra("wei.mark.standout.data", data)
                .putExtra("wei.mark.standout.fromCls", fromCls).putExtra("fromId", fromId)
    ...
    public PopupWindow getDropDown(final int id) {
        ...
        items.add(new DropDownListItem(android.R.drawable.ic_menu_close_clear_cancel, "Quit "
        ...
        final PopupWindow dropDown = new PopupWindow(scroller, StandOutLayoutParams.WRAP_CONTENT,
                StandOutLayoutParams.WRAP_CONTENT, true);

        for (final DropDownListItem item : items) {
            ViewGroup listItem = (ViewGroup) mLayoutInflater.inflate(R.layout.drop_down_list_item,
                    null);
    ...
    public final synchronized Window show(int id) {
        ...
        /*
         * // get the persistent notification SharedPreferences prefs =
         * PreferenceManager.getDefaultSharedPreferences(this); Notification
         * notification = getPersistentNotification(id); if
         * (!prefs.getBoolean("notification", true)) notification.icon =
         * R.drawable.nothing; // show the notification if (notification !=
         * null) { notification.flags = notification.flags |
         * Notification.FLAG_NO_CLEAR; // only show notification if not shown
         * before if (!startedForeground) { // tell Android system to show
         * notification if (id == 0) startForeground(getClass().hashCode() +
         * ONGOING_NOTIFICATION_ID, notification); startedForeground = true; }
         * else { // update notification if shown before if (id == 0)
         * mNotificationManager.notify(getClass().hashCode() +
         * ONGOING_NOTIFICATION_ID, notification); } } else { // notification
         * can only be null if it was provided before if (!startedForeground) {
    ...
    public final synchronized void reallyHide(int id) {
        final Window window = getWindow(id);
        if (window != null && window.getVisibility() != View.GONE) {
            window.setVisibility(View.GONE);
            Log.d(TAG, "reallyHide X " + id);
        }
        Log.d(TAG, "reallyHide " + id);
    }

    public final synchronized void reallyUnhide(int id) {
        final Window window = getWindow(id);
        if (window != null && window.getVisibility() != View.VISIBLE) {
            window.setVisibility(View.VISIBLE);
        }
        Log.d(TAG, "reallyUnhide " + id);
    }
     ...
     *            window.
     */
    public final void sendData(int fromId, Class<? extends StandOutWindow> toCls, int toId,
    ...
    public class StandOutLayoutParams extends WindowManager.LayoutParams {
        ...
        public StandOutLayoutParams(int id) {
            ...
            super(200, 200,
                    PreferenceManager.getDefaultSharedPreferences(StandOutWindow.this).getInt(
                            "type", LayoutParams.TYPE_SYSTEM_OVERLAY),
                    PreferenceManager.getDefaultSharedPreferences(StandOutWindow.this).getInt(
                            "flags", StandOutLayoutParams.FLAG_SHOW_WHEN_LOCKED),
         ...
         * @param w The width of the window.
         * @param h The height of the window.
         * @param gravity The gravity of the window.
         */
         ...
         */
        public StandOutLayoutParams(int id, int w, int h, int xpos, int ypos, int minWidth,
                int minHeight) {
